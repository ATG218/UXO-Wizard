...
class PathVisualize(ScriptInterface):
    ...
    def execute(self, data: pd.DataFrame, params: Dict[str, Any],
        ...
        df = data.copy()
        result = ProcessingResult(success=True, data=data, processing_script=self.name)
        ...
        # Drop rows where essential coordinates are missing for visualization
        ...
        if params.get('output_options', {}).get('generate_stats_file', {}).get('value', True):
            ...
            # Create a unique filename for the stats file
            ...
            result.metadata['statistics_file'] = stats_filename


        # --- Plotting ---
        if params.get('visualization_options', {}).get('generate_plots', {}).get('value', True):
            fig, axs = plt.subplots(2, 2, figsize=(18, 16)) # Changed to 2x2 for better layout
            fig.suptitle(params.get('visualization_options', {}).get('plot_title', {}).get('value'), fontsize=20)
        ...
        # --- Layer Generation ---
        # Ensure layer is created from the cleaned dataframe with valid coordinates
        layer_df = vis_df[['Latitude [Decimal Degrees]', 'Longitude [Decimal Degrees]']].copy()
        
        # Create a unique layer name with a timestamp
        ...
        result.add_layer_output(
            ...
            data=layer_df,
            style_info={
                'line_color': '#007BFF', # A slightly different blue
                'line_width': 2.5,
                'line_opacity': 0.9
            },
            metadata={
                'layer_name': layer_name,
                'description': 'Drone survey flight path from raw data.',
