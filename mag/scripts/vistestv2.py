#!/usr/bin/env python3
"""
Magnetic Field Data Processing Pipeline (formerly vistestv2.py)
================================================================
    ...
    if INTERPOLATE_FIELD:
        ...
        for method_key, config in interp_configs.items():
            ...
            try:
                ...
                field_fg = folium.FeatureGroup(name=f"{config['name']} ({data_name})")
                ...
                for lat, lon, val in zip(LonGrid_final.ravel(), LatGrid_final.ravel(), Zi_method.ravel()):
                    if not np.isnan(val) and valid_points < 5000:  # Limit points for performance
                        folium.CircleMarker(
                ...
                # Add interpolation layer to final map
...
print("\n🗺 Interactive maps include toggleable layers for:")
print("  • Different interpolation methods applied to filtered data")
