...
CSV_PATH = '/Users/aleksandergarbuz/Downloads/data/20250610_152044_MWALK_#0122_processed_20250613_100409.csv'

# List of cutoffs - empty list will trigger interactive selection
...
PLOT_UTM_TIME = True  # Plot UTM paths with time progression
...
PLOT_INTERPOLATION = False  # Plot of interpolated residual field data
PLOT_FILTER_MASKS = False  # Plot showing which segments were excluded by which filtering criteria
...
def plot_interpolate_residuals(df, output_dir, enable=True, cut_beginning_pct=0, cut_end_pct=0, middlecut1=0, middlecut2=0):
    ...
    # Validate cutting percentages - ensure they're between 0-100%
    cut_beginning_pct = max(0, min(cut_beginning_pct, 100))
    cut_end_pct = max(0, min(cut_end_pct, 100))
...
def interactive_cutoff_selection(df):
    """Interactive GUI for selecting cutoff points on the UTM path
    
    This function opens a plot showing the UTM path and allows the user to:
    1. Use sliders to trim beginning and end portions of the data
    2. Place markers to cut out middle sections
    ...
    -------
    list
        List containing [beginning_pct, end_pct, [middle_cut_points...]]
    """
    plt.close('all')  # Close any existing plots
    
    # Create a simple data representation for the path
    easting = df['UTM_Easting'].values
    northing = df['UTM_Northing'].values
    time_pct = np.linspace(0, 100, len(df))
    
    # Create the figure and axes for the main plot and sliders
    fig = plt.figure(figsize=(12, 10))
    ax_main = fig.add_axes([0.1, 0.25, 0.8, 0.65])  # Main plotting area
    ax_begin = fig.add_axes([0.25, 0.15, 0.65, 0.03])  # Begin slider
    ax_end = fig.add_axes([0.25, 0.1, 0.65, 0.03])  # End slider
    ax_add_marker = fig.add_axes([0.25, 0.05, 0.15, 0.03])  # Add marker button
    ax_reset_markers = fig.add_axes([0.45, 0.05, 0.15, 0.03])  # Reset markers button
    ax_apply = fig.add_axes([0.65, 0.05, 0.15, 0.03])  # Apply button
    
    # Set axis labels and title
    ax_main.set_xlabel('UTM Easting (m)')
    ax_main.set_ylabel('UTM Northing (m)')
    ax_main.set_title('Select Cutoff Points for Magnetic Data')
    ax_main.grid(True)
    
    # Set equal aspect to maintain correct scale
    ax_main.axis('equal')
    
    # Create path segments that will be colored differently
    # Default is all data included (before any cutoffs)
    path_included, = ax_main.plot([], [], 'g-', lw=2.5, label='Included Data')
    path_excluded_start, = ax_main.plot([], [], 'r-', lw=1.5, alpha=0.7, label='Excluded (Start)')
    path_excluded_end, = ax_main.plot([], [], 'r-', lw=1.5, alpha=0.7, label='Excluded (End)')
    
    # Add markers for start and end points
    ax_main.plot(easting[0], northing[0], 'go', markersize=10, label='Start')
    ax_main.plot(easting[-1], northing[-1], 'bo', markersize=10, label='End')
    
    # Create a reference line for all data (lightweight, only for context)
    path_all, = ax_main.plot(easting, northing, 'k-', lw=0.7, alpha=0.3, zorder=1, label='All Data')
    
    # Legend
    ax_main.legend(loc='upper right', fontsize=9)
    
    # Start and end sliders
    slider_begin = Slider(ax_begin, 'Begin Cut %', 0, 50, valinit=0, valstep=0.25)
    slider_end = Slider(ax_end, 'End Cut %', 0, 50, valinit=0, valstep=0.25)
    
    # Create buttons
    btn_add_marker = Button(ax_add_marker, 'Add Marker')
    btn_reset_markers = Button(ax_reset_markers, 'Reset Markers')
    btn_apply = Button(ax_apply, 'Apply Cutoffs')
    
    # Store cutoff visualization elements
    begin_line = ax_main.axvline(easting[0], color='g', linestyle='--', lw=2, zorder=4)
    end_line = ax_main.axvline(easting[-1], color='r', linestyle='--', lw=2, zorder=4)
    
    # Store state for middle cuts
    middle_markers = []
    middle_cuts = []
    marker_pairs = []
    marker_lines = []
    ...
    def update_slider(val):
        ...
        begin_pct = slider_begin.val
        end_pct = slider_end.val
        
        # Calculate indices based on percentages
        begin_idx = int(len(df) * begin_pct / 100)
        end_idx = int(len(df) * (1 - end_pct / 100))
        
        # Update vertical lines
        if begin_idx < len(easting):
            begin_line.set_xdata([easting[begin_idx], easting[begin_idx]])
        if end_idx < len(easting):
            end_line.set_xdata([easting[end_idx], easting[end_idx]])
        
        # Update path segments with appropriate colors
        # Excluded start segment
        if begin_idx > 0:
            path_excluded_start.set_data(easting[:begin_idx], northing[:begin_idx])
        else:
            path_excluded_start.set_data([], [])
            
        # Excluded end segment
        if end_idx < len(easting) - 1:
            path_excluded_end.set_data(easting[end_idx:], northing[end_idx:])
        else:
            path_excluded_end.set_data([], [])
            
        # Included segment (considers middle cuts too)
        included_mask = np.ones(len(easting), dtype=bool)
        included_mask[:begin_idx] = False  # Exclude beginning
        if end_idx < len(easting):
            included_mask[end_idx:] = False  # Exclude end
            
        # Also exclude middle sections based on marker pairs
        # Middle cuts should be processed in pairs, ensuring proper order
        for i in range(0, len(middle_cuts), 2):
            if i+1 < len(middle_cuts):
                idx1 = middle_cuts[i]
                idx2 = middle_cuts[i+1]
                # Ensure indices are in ascending order
                start_idx = min(idx1, idx2)
                end_idx = max(idx1, idx2)
                # Mark this region as excluded
                included_mask[start_idx:end_idx] = False
                
        # Get the included points
        included_easting = easting[included_mask]
        included_northing = northing[included_mask]
        path_included.set_data(included_easting, included_northing)
        
        # Update the plot
        fig.canvas.draw_idle()
    
    # Add marker mode flag
    add_marker_mode = [False]
    
    def toggle_add_marker(event):
        add_marker_mode[0] = not add_marker_mode[0]
        if add_marker_mode[0]:
            btn_add_marker.label.set_text('Adding...')
        else:
            btn_add_marker.label.set_text('Add Marker')
    
    # Add marker when clicking on plot in add marker mode
    def on_click(event):
        if event.inaxes == ax_main and add_marker_mode[0]:
            # Calculate percentage position by finding nearest point in path
            clicked_x, clicked_y = event.xdata, event.ydata
            distances = np.sqrt((easting - clicked_x)**2 + (northing - clicked_y)**2)
            nearest_idx = np.argmin(distances)
            marker_pct = time_pct[nearest_idx]
            
            # Add marker to list
            middle_markers.append(marker_pct)
            middle_cuts.append(nearest_idx)
            
            # Add visual marker
            marker = ax_main.plot(easting[nearest_idx], northing[nearest_idx], 'mx', markersize=10)[0]
            marker_lines.append(marker)
            
            # Display marker information
            print(f"Marker added at position {marker_pct:.2f}% (index {nearest_idx})")
            
            # If we have an even number of markers, draw a highlighted section
            if len(middle_markers) % 2 == 0 and len(middle_markers) >= 2:
                # Get the last two markers
                pct1 = middle_markers[-2]
                pct2 = middle_markers[-1]
                idx1 = middle_cuts[-2]
                idx2 = middle_cuts[-1]
                ...
                # Draw highlighted section (red section for excluded data)
                excluded_segment = ax_main.plot(easting[idx1:idx2], northing[idx1:idx2], 'r-', lw=3, alpha=0.7)[0]
                marker_lines.append(excluded_segment)
                
                # Add annotation to make it clear this is cut out
                mid_x = (easting[idx1] + easting[idx2]) / 2
                mid_y = (northing[idx1] + northing[idx2]) / 2
                cut_annotation = ax_main.annotate('CUT', xy=(mid_x, mid_y), 
                                           xytext=(0, 0), textcoords='offset points',
                                           color='red', fontweight='bold', fontsize=10,
                                           ha='center', va='center',
                                           bbox=dict(boxstyle='round,pad=0.3', fc='white', alpha=0.7))
                marker_lines.append(cut_annotation)
                
                # Store the correct order in marker_pairs
                print(f"Middle section to cut: {pct1:.2f}% - {pct2:.2f}%")
                marker_pairs.append((pct1, pct2))
                
            # Update the path visualization to reflect this new marker
            update_slider(0)  # Pass any value, it uses the current slider values
            
            # Turn off marker mode after placing
            add_marker_mode[0] = False
            btn_add_marker.label.set_text('Add Marker')
            
            fig.canvas.draw_idle()
            
    # Reset marker button callback
    def reset_markers(event):
        nonlocal middle_markers, middle_cuts, marker_pairs, marker_lines
        
        # Clear all markers and their visual elements
        middle_markers = []
        middle_cuts = []
        marker_pairs = []
        ...
        # Update the path visualization (remove middle cut segments)
        update_slider(0)  # Pass any value, it uses the current slider values
        
        fig.canvas.draw_idle()
        print("All markers have been reset")
    
    # Result container for the cutoffs values
    result = [0, 0, []]
    ...
    def apply_cutoffs(event):
        ...
        # Process middle cuts - these are pairs of percentages defining regions to cut out
        # Each pair is (start_pct, end_pct) for a region that should be excluded
        middle_cut_pairs = []
        for start_pct, end_pct in marker_pairs:
            # Add both percentages to the list
            middle_cut_pairs.append(start_pct)
            middle_cut_pairs.append(end_pct)
        
        # Update the result
        result[0] = begin_pct
        result[1] = end_pct
        result[2] = middle_cut_pairs
        
        # Display the selected cutoffs
        print("\nSelected Cutoffs:")
        print(f"Begin: {begin_pct}%")
        print(f"End: {end_pct}%")
        if middle_cut_pairs:
            print("Middle cuts:", middle_cut_pairs)
            for i in range(0, len(middle_cut_pairs), 2):
                if i+1 < len(middle_cut_pairs):
                    print(f"  - Cut from {middle_cut_pairs[i]:.2f}% to {middle_cut_pairs[i+1]:.2f}%")
        else:
            print("No middle cuts selected")
        
        # Close the figure
        plt.close(fig)
    
    # Connect the update functions
    slider_begin.on_changed(update_slider)
    slider_end.on_changed(update_slider)
    btn_add_marker.on_clicked(toggle_add_marker)
    btn_reset_markers.on_clicked(reset_markers)
    btn_apply.on_clicked(apply_cutoffs)
    ...
    instruction_text = (
        "Instructions:\n"
        "1. Use sliders to trim beginning and end\n"
        "2. Click 'Add Marker' then click on path to place markers\n"
        "3. Place markers in pairs to define regions to cut out\n"
        "   (regions marked red with 'CUT' will be excluded)\n"
        "4. Click 'Apply Cutoffs' when done"
    )
    ax_main.text(0.02, 0.02, instruction_text, transform=ax_main.transAxes,
               bbox=dict(boxstyle='round', facecolor='white', alpha=0.8))
    
    plt.show(block=True)  # Show the plot and wait for user interaction
...
def main():
    ...
    # Get cutoff values either from CUTOFFS list or interactive selection
    if not CUTOFFS:  # If CUTOFFS list is empty
        print("No cutoff values provided. Opening interactive cutoff selection.")
        cutoff_values = interactive_cutoff_selection(df)
        print(f"Using interactively selected cutoffs: {cutoff_values}")
        # Extract cutoffs
        cut_beginning_pct = cutoff_values[0]
        cut_end_pct = cutoff_values[1] 
        middle_cuts = cutoff_values[2]
    else:  # Use predefined cutoffs from the list
        print(f"Using predefined cutoffs: {CUTOFFS}")
    ...
    # Determine middle cut points for interpolation function
    # plot_interpolate_residuals uses separate middlecut1 and middlecut2 parameters
    # for a single middle section, so we use the first pair if available
    middlecut1 = 0
    middlecut2 = 0
    if middle_cuts and len(middle_cuts) >= 2:
        middlecut1 = middle_cuts[0]
        middlecut2 = middle_cuts[1]
    
    print(f"\nCutoff values to be applied:")
    print(f"Beginning: {cut_beginning_pct}%")
    print(f"End: {cut_end_pct}%")
    if middle_cuts:
        for i in range(0, len(middle_cuts), 2):
            if i+1 < len(middle_cuts):
                print(f"Middle cut {i//2+1}: {middle_cuts[i]}% to {middle_cuts[i+1]}%")
    
    # Create the hard cutoff mask that will be used by all visualization functions
    total_rows = len(df)
    hard_cutoff_mask = np.ones(total_rows, dtype=bool)
    
    # Apply beginning and end cutoffs
    if cut_beginning_pct > 0:
        cut_beginning_idx = int(total_rows * (cut_beginning_pct / 100))
        hard_cutoff_mask[:cut_beginning_idx] = False
        print(f"Applied beginning cutoff: removed first {cut_beginning_idx} points")
        
    if cut_end_pct > 0:
        cut_end_idx = int(total_rows * (1 - cut_end_pct / 100))
        hard_cutoff_mask[cut_end_idx:] = False
        print(f"Applied end cutoff: removed last {total_rows - cut_end_idx} points")
    
    # Apply middle cutoffs (if provided)
    for i in range(0, len(middle_cuts), 2):
        if i+1 < len(middle_cuts):
            ...
            # Ensure proper order (smaller percentage first)
            if start_pct > end_pct:
                start_pct, end_pct = end_pct, start_pct
                
            middle_cut_start = int(total_rows * (start_pct / 100))
            middle_cut_end = int(total_rows * (end_pct / 100))
            
            # Apply the middle cut
            hard_cutoff_mask[middle_cut_start:middle_cut_end] = False
            cut_percentage = (end_pct - start_pct)
            print(f"Applied middle cutoff: removed {middle_cut_end - middle_cut_start} points from {middle_cut_start} to {middle_cut_end}")
            print(f"  From {start_pct:.2f}% to {end_pct:.2f}% ({cut_percentage:.2f}% of data)")
            
            # Also update middlecut1 and middlecut2 if this is the first pair
            if i == 0:
                middlecut1 = start_pct
                middlecut2 = end_pct
    
    # Create a filtered dataframe with the hard cutoffs applied
    df_filtered = df[hard_cutoff_mask].copy()
    print(f"Original data points: {total_rows}, After cutoffs: {len(df_filtered)} ({len(df_filtered)/total_rows*100:.1f}%)")
    
    # Run the plots with the filtered data for uniform visualization
    # The intelligent filtering is only applied within plot_interpolate_residuals
    plot_utm_paths(df_filtered, output_dir, enable=PLOT_UTM_PATHS)
    plot_utm_paths_over_time(df_filtered, output_dir, enable=PLOT_UTM_TIME)
    plot_magnetic_field_over_time(df_filtered, output_dir, enable=PLOT_MAG_FIELD)
    plot_residuals(df_filtered, output_dir, enable=PLOT_RESIDUALS)
    plot_utm_components_over_time(df_filtered, output_dir, enable=PLOT_UTM_COMPONENTS)
    plot_interactive_utm_paths(df_filtered, output_dir, html_dir, enable=PLOT_INTERACTIVE_UTM)
    
    # For interpolate_residuals, we still pass the full dataset and cutoff parameters
    # as it performs its own intelligent filtering in addition to the hard cutoffs
    # Make sure the percentages and not indices are passed
    print(f"Passing to interpolation: begin={cut_beginning_pct}%, end={cut_end_pct}%, middle: {middlecut1}% to {middlecut2}%")
